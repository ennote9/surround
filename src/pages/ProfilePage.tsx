...
function getProfileInitials(displayName: string | null | undefined, email?: string): string {
  const source = displayName?.trim() || email?.split("@")[0]?.trim() || "U"
  const parts = source
    .split(/\s+/)
    .map((part) => part.trim())
    .filter(Boolean)
...
function getCloudStatusMeta(
  ...
  if (status === "saved") {
    return {
      label: "Сохранено",
      detail: savedAt ? `в ${formatSavedTime(savedAt)}` : "в облаке",
      tone: "green" as const,
    }
  }
  if (status === "error") {
    return {
      label: "Ошибка",
      detail: "Нужно проверить синхронизацию",
      tone: "red" as const,
    }
  }
  return {
    label: "Готово",
    detail: "Облачное сохранение включено",
    tone: "slate" as const,
  }
}
      ...
      <div className="space-y-6 md:hidden">
        <header className="min-w-0">
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            Личные данные, синхронизация и безопасность
          </p>
        </header>

        <section className="overflow-hidden rounded-[28px] border border-blue-200/70 bg-gradient-to-br from-blue-50 via-white to-indigo-50 p-5 shadow-sm dark:border-blue-500/20 dark:from-blue-500/10 dark:via-slate-900 dark:to-indigo-500/10">
          <div className="flex items-center gap-4">
            <div className="flex size-16 shrink-0 items-center justify-center rounded-[22px] bg-blue-600 text-xl font-semibold text-white shadow-lg shadow-blue-600/20">
              {getProfileInitials(profile?.displayName, user.email ?? undefined)}
            </div>

            <div className="min-w-0 flex-1">
              <h2 className="truncate text-xl font-semibold tracking-tight text-slate-950 dark:text-white">
                {profile?.displayName?.trim() || user.email?.split("@")[0] || "Пользователь"}
              </h2>
              ...
              <div className="mt-2 flex flex-wrap gap-1.5">
                <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2 py-1 text-[10px] font-medium text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300">
                  ...
                  {user.email_confirmed_at ? "Email подтверждён" : "Email не подтверждён"}
                </span>
          ...
          {(() => {
            const syncMeta = getCloudStatusMeta(
              status,
              status === "saved" ? cloudSaveState.savedAt : undefined,
            )
            return (
              <div className="mt-5 flex items-center justify-between gap-3 rounded-2xl bg-white/70 px-3.5 py-3 ring-1 ring-slate-200/70 dark:bg-slate-950/40 dark:ring-slate-700/70">
        ...
        <section className="space-y-3.5">
          ...
          <div className="rounded-[24px] border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            {!profileLoading && profileError ? (
              <div className="rounded-2xl border border-red-200 bg-red-50 p-3 dark:border-red-900/50 dark:bg-red-950/30">
                ...
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  className="mt-3 w-full dark:border-red-900/60 dark:bg-transparent"
            ...
            {!profileLoading && profile && !profileError ? (
              <div className="space-y-3">
                <div className="grid gap-2">
                  <Label htmlFor="profile-display-name-mobile">Имя в приложении</Label>
                  <Input
                    id="profile-display-name-mobile"
                    type="text"
                    ...
                    placeholder="Например, Шохрух"
                    maxLength={MAX_DISPLAY_NAME_LENGTH}
                    className="min-h-11 border-slate-300 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100"
                    autoComplete="nickname"
                  />
                  <p className="text-xs leading-5 text-slate-500 dark:text-slate-400">
                    Это имя используется внутри Life Progress OS. Email аккаунта не изменится.
                  </p>
                </div>
                ...
                <Button
                  type="button"
                  className="min-h-11 w-full rounded-xl bg-blue-600 text-white hover:bg-blue-700"
        ...
        <section className="space-y-3.5">
          ...
          <div className="overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <Link
              to="/settings"
              className="flex min-h-16 items-center gap-3 border-b border-slate-100 px-4 py-3.5 transition-colors hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800/70"
            >
            ...
            <button
              ...
              className="flex min-h-16 w-full items-center gap-3 border-b border-slate-100 px-4 py-3.5 text-left transition-colors hover:bg-slate-50 disabled:opacity-50 dark:border-slate-800 dark:hover:bg-slate-800/70"
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="text-sm font-semibold text-slate-950 dark:text-slate-100">
                  {passwordResetSending ? "Отправляем письмо…" : "Сбросить пароль"}
                </p>
                <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                  Получить ссылку на {accountEmailForReset || "email"}
                </p>
              </div>
              <ChevronRight className="size-4 shrink-0 text-slate-300 dark:text-slate-600" aria-hidden />
            </button>
            ...
            <details className="group">
              <summary className="flex min-h-16 cursor-pointer list-none items-center gap-3 px-4 py-3.5 [&::-webkit-details-marker]:hidden">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                    ID, провайдер и даты аккаунта
                  </p>
                </div>
                <ChevronRight className="size-4 shrink-0 text-slate-300 transition-transform group-open:rotate-90 dark:text-slate-600" aria-hidden />
              </summary>

              <div className="border-t border-slate-100 px-4 pb-4 pt-3 dark:border-slate-800">
                <div className="space-y-3 rounded-2xl bg-slate-50 p-3.5 dark:bg-slate-950/55">
                  <div>
                    <p className="text-[10px] uppercase tracking-wide text-slate-400">User ID</p>
                    <code className="mt-1 block break-all font-mono text-xs leading-5 text-slate-700 dark:text-slate-300">
                      {user.id}
                    </code>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <p className="text-[10px] uppercase tracking-wide text-slate-400">Создан</p>
                      <p className="mt-1 text-xs font-medium text-slate-700 dark:text-slate-300">
                        {formatDateTime(user.created_at)}
                      </p>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase tracking-wide text-slate-400">Последний вход</p>
                      <p className="mt-1 text-xs font-medium text-slate-700 dark:text-slate-300">
                        {user.last_sign_in_at ? formatDateTime(user.last_sign_in_at) : "—"}
                      </p>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase tracking-wide text-slate-400">Провайдер</p>
                      <p className="mt-1 text-xs font-medium text-slate-700 dark:text-slate-300">
                        {getProviderLabel(user)}
                      </p>
                    </div>
          ...
          {status === "error" ? (
            <div className="rounded-2xl border border-red-200 bg-red-50 p-3.5 dark:border-red-900/50 dark:bg-red-950/30">
              ...
              <Button
                ...
                variant="outline"
                size="sm"
                className="mt-3 w-full border-red-200 dark:border-red-900/60 dark:bg-transparent"
        ...
        <section className="space-y-3">
          <button
            type="button"
            className="flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl border border-red-200 bg-red-50 px-4 text-sm font-semibold text-red-700 transition-colors hover:bg-red-100 disabled:opacity-50 dark:border-red-900/60 dark:bg-red-950/25 dark:text-red-300 dark:hover:bg-red-950/40"
          ...
          <p className="text-center text-[11px] text-slate-400">
            Аккаунт останется в системе. Будет завершена только текущая сессия.
          </p>
        </section>
      </div>
